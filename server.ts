...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-lobby', ({ lobbyCode, userId, username }) => {
      ...
      const newUser: User = { id: userId, name: username, isHost: false };

      // ✅ SET HOST FIRST, before any lobby modifications
      if (getLobbyUsers(lobbyCode).length === 0) {
      ...
      // ✅ Now emit - hostId will exist
      const lobbyData = {
        lobbyCode,
        users: getLobbyUsers(lobbyCode),
        hostId: lobbyHosts.get(lobbyCode) || userId  // Fallback
      };
...
socket.on('finish-quiz', ({ lobbyCode }) => {
  ...
  // Optional: Cleanup lobby after delay
  setTimeout(() => {
    ...
    console.log(`🧹 ${lobbyCode}: Lobby cleaned up`);
  }, 30000);
});
